 ...
 * @version 1.0
 */
public class TestMapper extends Mapper<LongWritable, Text, OutFieldsBaseModel, OutFieldsBaseModel> {
	...
	private static int MAX_LENGTH = 65536; // 理论最大值65536
	...
	protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		System.out.println("pkgName.getBytes(UTF8).length:"+pkgName.getBytes(UTF8).length);
		System.out.println("appName.getBytes(UTF8).length:"+appName.getBytes(UTF8).length);
		System.out.println("version.getBytes(UTF8).length:"+version.getBytes(UTF8).length);
