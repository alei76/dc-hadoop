...
public class UserInfoRollingReviseReducer
		extends
	...
	protected void reduce(OutFieldsBaseModel key,
		...
		if (paymentArray != null) { //
			...
			if (userInfoRollingLog.getPlayerDayInfo().getFirstPayDate() == 0) {
				...
				// 增加是否新增付费玩家统计，与业务无关
				if (MRConstants.ALL_GAMESERVER.equals(payGameServer)) {
					context.getCounter("PAY_PALYER_COUNTER", payAppId + "_NEW")
							.increment(1);
				}
			} else {
				if (MRConstants.ALL_GAMESERVER.equals(payGameServer)) {
					context.getCounter("PAY_PALYER_COUNTER", payAppId + "_OLD")
							.increment(1);
				}
			}
		...
		if (null != onlineDayLog && null != onlineArray) {
			statPlayerOnlineInfo(onlineArray, isNewPlayer, isPayToday);
		}
		// 输出新增玩家、新增付费玩家
		// 新增付费玩家
	...
	private void statUserHabits(int statDate,
		...
		if (onlineTime > 0 && totalLoginTimes > 0) {
			boolean isNewUser = statDate == userInfoRollingLog
					.getPlayerDayInfo().getFirstLoginDate();
	...
	private int getStatDate() {
		...
		// 修正计算，数据时间需减 2
		calendar.add(Calendar.DAY_OF_MONTH, -2);// 结算时间默认调度时间的前一天
	...
	private void payTimeInterval(Context context, int statDate,
			...
			UserInfoRollingLog userHistoryInfo, String[] paymentArray,
		...
		if (historyPayTimes >= 3 || null == userHistoryInfo
				|| null == paymentArray || null == onlineRecords
		...
		// (目前业务需求值统计前三次充值时间间隔)
		String[] paymentRecords = paymentArray[paymentArray.length - 1]
				.split(",");
		...
		int[] payTimeArr = new int[payTimeSet.size() > 3 ? 3 : payTimeSet
				.size()];
		int i = 0;
	...
	private void statUserFirstPay(int statDate,
		...
		int onlineDayRange = EnumConstants
				.getFirstPayGameDaysRange(totalOnlineDay);
		// int onlineTimeRange =
		// EnumConstants.getGameTimeRange(totalOnlineTime);
		...
		// 输出结果
		// 首付时玩家游戏天数
		String[] gameDaysArr = new String[] { userInfoRollingLog.getAppID(),
				userInfoRollingLog.getPlatform(),
				userInfoRollingLog.getPlayerDayInfo().getChannel(),
				userInfoRollingLog.getPlayerDayInfo().getGameRegion(),
				Constants.DIMENSION_PLAYER_DAYS, "" + onlineDayRange };
		...
		// 首付时玩家游戏时长
		String[] gameTimeArr = new String[] { userInfoRollingLog.getAppID(),
				userInfoRollingLog.getPlatform(),
				userInfoRollingLog.getPlayerDayInfo().getChannel(),
				userInfoRollingLog.getPlayerDayInfo().getGameRegion(),
				Constants.DIMENSION_PLAYER_ONLINETIME, "" + onlineTimeRange };
		...
		// 首付时玩家游戏等级
		String[] gameLevelArr = new String[] { userInfoRollingLog.getAppID(),
				userInfoRollingLog.getPlatform(),
				userInfoRollingLog.getPlayerDayInfo().getChannel(),
				userInfoRollingLog.getPlayerDayInfo().getGameRegion(),
				Constants.DIMENSION_PLAYER_LEVEL, "" + levelRange };
	...
	// 输出激活新增信息
	private void statNewActDevicePlayer(String[] regArray,
			UserInfoRollingLog userInfoRollingLog, int statDate)
			throws IOException, InterruptedException {
		...
		if (actTime > 0) {
			...
			// 只统计激活设备及激活设备中的新增玩家
			// 所以只输出激活时间大于 0 的记录
			String[] newActDevicePlayer = { userInfoRollingLog.getAppID(),
					userInfoRollingLog.getPlatform(),
					regArray[3], // channel
	...
	// 输出玩家在线信息，并标识是否新增、付费
	private void statPlayerOnlineInfo(String[] onlineArray,
			boolean isNewPlayer, boolean isPayToday) throws IOException,
			InterruptedException {
	...
	// 输出新增玩家修订
	private void statNewAddPlayerRevise(OnlineDayLog onlineDayLog,
			String[] onlineArray, int statDate) throws IOException,
			InterruptedException {
		int minLoginTime = Integer.MAX_VALUE;
		...
		for (String record : records) {
			...
			if (arr.length > 1) {
				int loginTime = StringUtil.convertInt(arr[0], 0);
				if (loginTime <= 0) { // 登录时间不合法，则过滤
					continue;
				}
				minLoginTime = Math.min(minLoginTime, loginTime);
			}
		}

		// 最小登录时间在昨天(大于昨天零点零分小于当天零点零分)则输出给昨天重新计算修正
		if (minLoginTime >= (statDate - 24 * 60 * 60)
				&& minLoginTime < statDate) {
			OutFieldsBaseModel outKey = new OutFieldsBaseModel();
			outKey.setSuffix(Constants.SUFFIX_NEW_PLAYER_REVISE);
			outKey.setOutFields(onlineArray);
			context.write(outKey, NullWritable.get());
		}
	}
