 ...
 *           isFirstPay, weeklogintimes,
 *           weekonlinetime,weekonlineday,weekcurrencyamount,weekpaytimes
 */

public class WeekUserHabitsMapper extends
	...
	protected void map(LongWritable key, Text value, Context context)
		...
		// B .. 统计周游戏天数，周游戏次数，周游戏时长分布
		int weekLoginTimesItval = EnumConstants
				.getItval4WeekLoginTimes(loginTimes);
		int weekOnlineTimeItval = EnumConstants
				.getItval4WeekOnlineTime(onlineTime);

		if (isFirstLogin) {
			// 周游戏天数区间
			mapKeyObj.setOutFields(new String[] { appid, platform, channel,
					gameRegion, Constants.PLAYER_TYPE_NEWADD,
					Constants.DIMENSION_PLAYER_DAYS, onlineDay + "", });
		...
		if (currencyAmount > 0) {
			...
			mapKeyObj.setOutFields(new String[] { 
					...
					Constants.DIMENSION_WEEK_PayTimes, 
					payTimesRange + ""
			});
			mapValueObj.setOutFields(new String[] { "B" });
			context.write(mapKeyObj, mapValueObj);
			...
			mapKeyObj.setOutFields(new String[] { 
					...
					gameRegion, 
					Constants.PLAYER_TYPE_PAYMENT,
					Constants.DIMENSION_WEEK_PayAmount, 
					payAmountRange + "" 
			});
			mapValueObj.setOutFields(new String[] { "B" });
			context.write(mapKeyObj, mapValueObj);
