...
import net.digitcube.hadoop.mapreduce.hbase.PlayerBasicInfo;
...
public class UserInfoRollingHour2Reducer
		extends
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		htable = conn.getTable(PlayerBasicInfo.TB_PLAYER_BASIC_INFO);
	}
	...
	protected void reduce(OutFieldsBaseModel key,
		...
		setPlayerBasicInfoFromHBase(keyArray, userInfoRollingLog);
	...
	private void setPlayerBasicInfoFromHBase(String[] keyArr, UserInfoRollingLog userRollInfo){
		String appId = keyArr[0]; 
		String platform = keyArr[1];
		String accountId = keyArr[2];
		String gameServer = keyArr[3];
		...
		try {
			...
			if(null != result){
				//HBase 滚存表中查到该玩家，把相关信息设到 userRollInfo 对象中
				PlayerBasicInfo basicInfo = new PlayerBasicInfo(result);
				userRollInfo.getPlayerDayInfo().setLevel(basicInfo.getPlayerLevel());
				userRollInfo.getPlayerDayInfo().setFirstLoginDate(basicInfo.getFirstLoginDay());
				userRollInfo.getPlayerDayInfo().setLastLoginDate(basicInfo.getLastLoginDay());
				userRollInfo.getPlayerDayInfo().setTotalOnlineDay(basicInfo.getTotalOnlineDays());
				userRollInfo.getPlayerDayInfo().setTotalOnlineTime(basicInfo.getTotalOnlineTime());
				userRollInfo.getPlayerDayInfo().setTotalLoginTimes(basicInfo.getTotalLoginTimes());
				userRollInfo.getPlayerDayInfo().setFirstPayDate(basicInfo.getFirstPayDay());
				userRollInfo.getPlayerDayInfo().setLastPayDate(basicInfo.getLastPayDay());
				userRollInfo.getPlayerDayInfo().setTotalCurrencyAmount(basicInfo.getTotalPayAmount());
				userRollInfo.getPlayerDayInfo().setTotalPayTimes(basicInfo.getTotalPayTimes());
			}
			
			//HBase 滚存表中没查到该玩家，说明是新增玩家
			//不设置任何信息到 userRollInfo，reduce 其它逻辑会将该玩家判断为新增玩家
			
		} catch (Throwable e) {
			//发生异常时，设置首登/首付日期为 1，避免reduce 其它逻辑会将该玩家判断为新增玩家
			userRollInfo.getPlayerDayInfo().setFirstLoginDate(1);
			userRollInfo.getPlayerDayInfo().setFirstPayDate(1);
			e.printStackTrace();
		}
	}
