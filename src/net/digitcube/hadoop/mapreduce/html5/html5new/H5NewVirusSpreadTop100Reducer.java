...
public class H5NewVirusSpreadTop100Reducer extends Reducer<OutFieldsBaseModel, OutFieldsBaseModel, OutFieldsBaseModel, OutFieldsBaseModel> {
	...
	private TreeSet<VirusSpreadInfo> onlineSpreadTop100Set = new TreeSet<VirusSpreadInfo>();
	
	@Override
	protected void reduce(OutFieldsBaseModel key, Iterable<OutFieldsBaseModel> values, Context context) throws IOException, InterruptedException {
		spreadTop100Set.clear();
		onlineSpreadTop100Set.clear();
		
		int totalParentNodes = 0;
		int totalChildNodes = 0;
		
		//当天活跃玩家传播的子节点数量
		int onlineSpreadNodes = 0;
		...
		for (OutFieldsBaseModel val : values) {
			...
			if(MRConstants.INVALID_PLACE_HOLDER_CHAR.equals(parentAccountId)){
				...
				int nodeCounts = StringUtil.convertInt(childNodeCount, 0);
				totalChildNodes += nodeCounts;
				...
				//处理活跃 K 系数
				//如果父节点当天是活跃的话，那么计算活跃玩家传播的子节点数
				if(Constants.DATA_FLAG_YES.equals(isOneLine)){
					onlineSpreadNodes += nodeCounts;
					if(onlineSpreadTop100Set.size() < spreadTopN){
						onlineSpreadTop100Set.add(spredInfo);
					}else if(null != onlineSpreadTop100Set.floor(spredInfo)){
						onlineSpreadTop100Set.remove(onlineSpreadTop100Set.first());
						onlineSpreadTop100Set.add(spredInfo);
					}
				}
		...
		for(VirusSpreadInfo info : spreadTop100Set){
			String[] valFields = new String[]{
					...
					info.totalPVs+"",
					info.uniqIps+"",
					Constants.K_TYPE_FACTORY+""
			};			
			valObj.setOutFields(valFields);
			key.setSuffix(Constants.SUFFIX_H5_NEW_VIRUS_SPREAD_TOP100);
			context.write(key, valObj);
		}
		
		//输出传播活跃k系数  TOP100 的详情
		for(VirusSpreadInfo info : onlineSpreadTop100Set){
			String[] valFields = new String[]{
					info.parentAccountId,
					info.childNodeCount+"",
					info.totalLoginTimes+"",
		...
		valObj.setOutFields(valFields);
		key.setSuffix(Constants.SUFFIX_H5_NEW_VIRUS_SPREAD_KFACTOR);
		context.write(key, valObj);
	}
